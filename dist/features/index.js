...
function findSwings(candles, lookback = 2) {
    ...
    for (let i = lookback; i < n - lookback; i++) {
        ...
        if (isHigh)
            highs.push({ price: c.high, barIndex: i });
        if (isLow)
            lows.push({ price: c.low, barIndex: i });
    }
    return { highs, lows };
}
// ── Trend features ────────────────────────────────────────────────────────────
function buildTrendFeatures(candles, ind, indicatorCfg = {}) {
    ...
    const emaAlignment = emaFastNow > emaSlowNow * 1.0001 ? 'bullish'
        : emaFastNow < emaSlowNow * 0.9999 ? 'bearish'
            : 'neutral';
    const rsiValue = ind.rsi14 ?? 50;
    const rsiZone = rsiValue < 35 ? 'oversold' : rsiValue > 65 ? 'overbought' : 'neutral';
    const adxValue = ind.adx?.adx ?? 0;
    const adxStrength = adxValue >= 35 ? 'strong' : adxValue >= 20 ? 'moderate' : 'weak';
    const macdBias = ind.macd
        ? ind.macd.histogram > 0 ? 'bullish' : ind.macd.histogram < 0 ? 'bearish' : 'neutral'
        : undefined;
    const psarBias = ind.psar
        ? ind.psar.bullish ? 'bullish' : 'bearish'
        : undefined;
    const mtfScore = ind.mtf?.confluence;
    const mtfAlignment = mtfScore == null ? 'neutral'
    ...
    else if (macdBias === 'bearish') {
        bearScore += 1;
    }
    if (psarBias === 'bullish') {
        bullScore += 1;
    }
    else if (psarBias === 'bearish') {
        bearScore += 1;
    }
    if (rsiValue > 55) {
        bullScore += 0.5;
    ...
    if (mtfScore != null && mtfScore > 0) {
        bullScore += Math.abs(mtfScore) * 0.5;
    }
    if (mtfScore != null && mtfScore < 0) {
        bearScore += Math.abs(mtfScore) * 0.5;
    }
    if (ind.obv?.rising) {
        bullScore += 0.5;
    }
    else if (ind.obv && !ind.obv.rising) {
        bearScore += 0.5;
    }
    const maxScore = 7; // sum of max possible contributions
    const net = bullScore - bearScore;
    const directionBias = net > 0.5 ? 'bullish' : net < -0.5 ? 'bearish' : 'neutral';
...
// ── Volatility features ───────────────────────────────────────────────────────
function buildVolatilityFeatures(candles, ind, point) {
    ...
    const volatilityPercentile = atrValues.length > 0
        ...
        : 50;
    const volatilityRegime = volatilityPercentile >= 85 ? 'abnormal'
        : volatilityPercentile >= 65 ? 'elevated'
            : volatilityPercentile >= 25 ? 'normal'
                : 'quiet';
    const price = candles[candles.length - 1].close;
    let keltnerPosition = undefined;
    if (ind.keltner) {
...
// ── Structure features ────────────────────────────────────────────────────────
function buildStructureFeatures(candles, ind) {
    const n = candles.length;
    ...
    if (price > lastHigh.price)
        bos = 'bullish';
    else if (price < lastLow.price)
        bos = 'bearish';
    // CHoCH: structure flip signal (simplified — opposite BOS in existing trend)
    let choch = null;
    if (trendDirection === 'downtrend' && bos === 'bullish')
        choch = 'bullish';
    if (trendDirection === 'uptrend' && bos === 'bearish')
        choch = 'bearish';
    ...
    const distToNearestSwing = Math.min(Math.abs(price - lastHigh.price), Math.abs(price - lastLow.price));
...
// ── Level features ────────────────────────────────────────────────────────────
function buildLevelFeatures(candles, ind, keyLevels) {
    const price = candles[candles.length - 1].close;
    // VWAP distance
    let vwapDistance = 0;
    let vwapSide = undefined;
    ...
    const nearest = (levels, fromPrice) => levels.length
...
// ── Session features ──────────────────────────────────────────────────────────
function buildSessionFeatures(ctx) {
    const s = ctx.session;
    if (!s) {
        return {
            activeSessions: [],
    ...
    if (s.isLondonNYOverlap)
        sessionQuality = 'optimal';
    else if (s.isOptimalSession)
        sessionQuality = 'favorable';
    else if (s.activeSessions.length > 0)
...
// ── Execution features ────────────────────────────────────────────────────────
function buildExecutionFeatures(ctx, point, symbol) {
...
function buildContextFeatures(ctx) {
    ...
    const dominantSentiment = newsCount === 0 ? 'none'
