...
// ── Consensus builder ─────────────────────────────────────────────────────────
function buildConsensus(core, markov, agent, scenarios, bayesian, kelly) {
    // Accumulate a numeric signal score (-3..+3) and a confidence vote count
    let score = 0; // positive = bullish, negative = bearish
    let votes = 0;
    const reasons = [];
    // Core MC (always)
    if (core.recommended === 'LONG') {
        score += 1;
        votes++;
    }
    if (core.recommended === 'SHORT') {
        score -= 1;
        votes++;
    }
    if (core.recommended !== 'HOLD')
        reasons.push(`MC→${core.recommended}`);
    // Markov
    if (markov) {
    ...
    // Crowd (contrarian)
    if (agent) {
        if (agent.contrarianSignal === 'FADE_LONGS') {
            score -= 1;
            votes++;
            reasons.push('Fade crowd longs');
        }
        if (agent.contrarianSignal === 'FADE_SHORTS') {
            score += 1;
            votes++;
            reasons.push('Fade crowd shorts');
        }
    }
    // Scenario: if worst case is HOLD or avoidTrading, penalise
    if (scenarios) {
        if (scenarios.avoidTrading) {
            score -= 1;
            votes++;
            reasons.push('All scenarios→HOLD');
        }
    ...
    if (bayesian) {
        if (bayesian.regimeShiftDetected) {
            score -= 0.5;
            reasons.push('Regime shift');
        }
        if (bayesian.confidence === 'VERY_HIGH')
            score *= 1.2;
    }
    // Kelly: NO_EDGE is a red flag
    if (kelly) {
        if (kelly.riskAssessment === 'NO_EDGE') {
            score -= 1;
            votes++;
            reasons.push('Kelly:NO_EDGE');
        }
    }
    // Normalise
    const normalised = votes > 0 ? score / votes : 0;
    const signal = (() => {
        if (scenarios?.avoidTrading && kelly?.riskAssessment === 'NO_EDGE')
            return 'AVOID';
        if (normalised >= 0.7)
            return 'STRONG_LONG';
        if (normalised >= 0.2)
    ...
    const confidence = (() => {
        if (!bayesian)
            return 'LOW';
        if (bayesian.confidence === 'VERY_HIGH' && Math.abs(normalised) > 0.5)
            return 'HIGH';
...
export function formatEnhancedMCBlock(result, pipSize, dp) {
    const lines = [];
    // Core table (always)
    ...
    if (result.bayesian) {
        ...
        if (b.regimeShiftDetected)
            lines.push(`    ⚠ REGIME SHIFT: ${b.regimeShiftReason}`);
    }
    // Significance
    if (result.significance) {
        ...
        if (s.tradesNeeded > 0)
            lines.push(`    ${s.tradesNeeded} more trades needed for 95% confidence.`);
    }
    // Kelly
    if (result.kelly) {
...
// ── Public entry point ────────────────────────────────────────────────────────
export async function runEnhancedMonteCarlo(inputs) {
...
//# sourceMappingURL=mc-orchestrator.js.map