...
export function detectSessionReversal(input) {
    ...
    if (structure.overextensionATR < 2.0) {
        return emptyCandidate(features.symbolKey, KEY, LABEL, [`Overextension ${structure.overextensionATR.toFixed(1)} ATR — need ≥ 2.0 for reversal setup`]);
    }
    ...
    if (hasDivergence) {
        if (indicators.divergence?.rsi)
            reasons.push(`RSI divergence: ${indicators.divergence.rsi}`);
        if (indicators.divergence?.macd)
            reasons.push(`MACD divergence: ${indicators.divergence.macd}`);
    }
    if (hasChoch)
        reasons.push(`CHoCH ${structure.choch} — structural confirmation`);
    const tags = [
        'session_reversal',
...
//# sourceMappingURL=sessionReversal.js.map