...
export function detectBreakoutRetest(input) {
    ...
    if (distToLevel > retestZone) {
        return emptyCandidate(features.symbolKey, KEY, LABEL, [`Price ${(distToLevel / atr).toFixed(1)} ATR from broken level — not retesting yet`]);
    }
    ...
    if (volatility.recentRangeExpansion)
        reasons.push('Range expansion on breakout bar');
    if (trend.macdBias === (isBull ? 'bullish' : 'bearish'))
        reasons.push('MACD confirming direction');
    const tags = [
        structure.bos,
