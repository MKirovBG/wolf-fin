...
// ── Price fetch ───────────────────────────────────────────────────────────────
async function fetchMidPrice(symbol, mt5AccountId) {
...
// ── Per-outcome check ─────────────────────────────────────────────────────────
async function checkOutcome(outcome, price) {
    ...
    if (outcome.status === 'entered') {
        ...
        const tp2Hit = outcome.tp2 != null && (isBuy ? price >= outcome.tp2 : price <= outcome.tp2);
        // Check TP1 hit
        const tp1Hit = outcome.tp1 != null && (isBuy ? price >= outcome.tp1 : price <= outcome.tp1);
...
// ── Start / stop ──────────────────────────────────────────────────────────────
let pollTimer = null;
export function startOutcomePoller() {
    if (pollTimer)
        return;
...
export function stopOutcomePoller() {
    if (pollTimer) {
