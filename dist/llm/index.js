...
export function getLLMProvider(config) {
    ...
    if (config.llmProvider === 'openai-subscription') {
        ...
        if (status === 'expired')
            throw new Error('OpenAI token expired — click Refresh on the Integrations page');
        if (status === 'missing')
            throw new Error('OpenAI account not connected — authorize on the Integrations page');
...
export function getOpenAIAccessToken() {
...
export function getOpenAITokenStatus() {
    const token = process.env.OPENAI_ACCESS_TOKEN;
    const expires = process.env.OPENAI_TOKEN_EXPIRES ? parseInt(process.env.OPENAI_TOKEN_EXPIRES) : 0;
    if (!token)
        return 'missing';
