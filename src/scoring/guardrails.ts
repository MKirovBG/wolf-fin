...
export function checkHardDisqualifiers(
  ...
  if (
    features.context.nextHighImpactMinutes !== null &&
  ...
  if (candidate.entryZone && candidate.stopLoss !== null) {
    ...
    if (atr > 0) {
      if (stopDist < atr * 0.25) fails.push(`Stop too tight: ${(stopDist / atr).toFixed(2)} ATR (minimum 0.25)`)
      if (stopDist > atr * 3.5)  fails.push(`Stop too wide: ${(stopDist / atr).toFixed(2)} ATR (maximum 3.5)`)
    }
  }

  // No valid direction
  if (!candidate.direction) fails.push('No trade direction — candidate incomplete')
