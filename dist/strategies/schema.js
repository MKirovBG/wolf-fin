...
export function validateStrategyDefinition(def) {
    const errors = [];
    if (!def || typeof def !== 'object')
        return ['Definition must be an object'];
    const d = def;
    if (!d.strategyKey || typeof d.strategyKey !== 'string')
        errors.push('strategyKey is required');
    if (!d.name || typeof d.name !== 'string')
        errors.push('name is required');
    if (!d.version || typeof d.version !== 'string')
        errors.push('version is required (semver)');
    if (!d.context || typeof d.context !== 'object') {
        errors.push('context object is required');
    }
    else {
        const ctx = d.context;
        if (!Array.isArray(ctx.allowedSessions))
            errors.push('context.allowedSessions must be array');
        if (!Array.isArray(ctx.allowedRegimes))
            errors.push('context.allowedRegimes must be array');
        if (typeof ctx.newsBufferMinutes !== 'number')
            errors.push('context.newsBufferMinutes must be number');
        if (typeof ctx.maxSpreadPips !== 'number')
            errors.push('context.maxSpreadPips must be number');
    }
    if (!Array.isArray(d.allowedDetectors) || d.allowedDetectors.length === 0) {
    ...
    if (!d.entryRules || typeof d.entryRules !== 'object') {
        ...
        const e = d.entryRules;
        if (typeof e.maxEntryWidthATR !== 'number')
            errors.push('entryRules.maxEntryWidthATR must be number');
        if (typeof e.requireConfirmation !== 'boolean')
            errors.push('entryRules.requireConfirmation must be boolean');
    }
    if (!d.riskRules || typeof d.riskRules !== 'object') {
        ...
        const r = d.riskRules;
        if (typeof r.minRR !== 'number' || r.minRR < 1)
            errors.push('riskRules.minRR must be >= 1');
        if (typeof r.maxStopATR !== 'number')
            errors.push('riskRules.maxStopATR must be number');
    }
    return errors;
}
