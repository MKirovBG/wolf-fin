...
export function aggregateByKey(candidates, getKey) {
    const map = {};
    for (const c of candidates) {
        const key = getKey(c);
        if (!map[key])
            map[key] = { trades: 0, wins: 0, losses: 0, scores: [], rMultiples: [] };
        const g = map[key];
        g.trades++;
