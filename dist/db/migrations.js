...
const MIGRATIONS = [
    ...
    },
    {
        ...
        run: (db) => {
            const cols = new Set(db.prepare('PRAGMA table_info(watch_symbols)').all().map(c => c.name));
            if (!cols.has('system_prompt'))
                db.exec(`ALTER TABLE watch_symbols ADD COLUMN system_prompt TEXT`);
            if (!cols.has('strategy'))
    ...
    },
    {
        ...
        run: (db) => {
            const cols = db.prepare('PRAGMA table_info(log_entries)').all();
    ...
    },
    {
      ...
      `);
            const count = db.prepare('SELECT COUNT(*) as n FROM strategies').get().n;
            if (count === 0) {
