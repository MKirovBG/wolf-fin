...
export function detectTrendPullback(input: DetectorInput): SetupCandidate {
  ...
  if (volatility.volatilityRegime === 'abnormal') disq.push('Abnormal volatility — range unstable')
  if (structure.choch !== null)              disq.push(`CHoCH ${structure.choch} — trend structure broken`)
  ...
  const trendMatchesStructure =
    (isBull && structure.trendDirection === 'uptrend') ||
