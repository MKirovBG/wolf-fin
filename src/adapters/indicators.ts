...
function swingHighIndices(values: number[], lookback = 3): number[] {
  const out: number[] = []
  for (let i = lookback; i < values.length - lookback; i++) {
    const win = values.slice(i - lookback, i + lookback + 1)
    if (values[i] === Math.max(...win)) out.push(i)
  }
  return out
}

function swingLowIndices(values: number[], lookback = 3): number[] {
  const out: number[] = []
  for (let i = lookback; i < values.length - lookback; i++) {
    const win = values.slice(i - lookback, i + lookback + 1)
    if (values[i] === Math.min(...win)) out.push(i)
  }
  return out
}

export function computeDivergence(candles: Candle[], period = 14): {
  rsi?: 'bullish' | 'bearish'
  ...
  const result: { rsi?: 'bullish' | 'bearish'; macd?: 'bullish' | 'bearish' } = {}
  ...
  if (priceHighs.length >= 2) {
    ...
    if (r1 >= 0 && r2 >= 0 && r2 < rsiVals.length) {
      if (prices[i2] > prices[i1] && rsiVals[r2] < rsiVals[r1]) result.rsi = 'bearish'
    }
  }
  ...
  if (!result.rsi && priceLows.length >= 2) {
    ...
    const r2 = i2 - offset
    if (r1 >= 0 && r2 >= 0 && r2 < rsiVals.length) {
      if (prices[i2] < prices[i1] && rsiVals[r2] > rsiVals[r1]) result.rsi = 'bullish'
    }
  }
  ...
  if (macdHist.length >= 6 && priceHighs.length >= 2) {
    ...
    if (m1 >= 0 && m2 >= 0 && m2 < macdHist.length) {
      if (prices[i2] > prices[i1] && macdHist[m2] < macdHist[m1]) result.macd = 'bearish'
    }
  }
  ...
  if (!result.macd && macdHist.length >= 6 && priceLows.length >= 2) {
    ...
    const m2 = i2 - macdOffset
    if (m1 >= 0 && m2 >= 0 && m2 < macdHist.length) {
      if (prices[i2] < prices[i1] && macdHist[m2] > macdHist[m1]) result.macd = 'bullish'
    }
  }
...
export function computeFibLevels(candles: Candle[]): Array<{ price: number; label: string }> {
...
export function computeIndicators(h1Candles: Candle[], cfg: IndicatorConfig = {}): Indicators {
  ...
  if (cfg.divergenceEnabled) result.divergence = computeDivergence(h1Candles)
  if (cfg.fibEnabled)        result.fib        = computeFibLevels(h1Candles)
  return result
}
...
function tfIndicators(candles: Candle[], includeEma50 = false): TFIndicators | undefined {
  if (candles.length < 15) return undefined  // not enough data
  const result: TFIndicators = {
...
export function computeMultiTFIndicators(
  m15Candles: Candle[],
  h1Candles: Candle[],
  h4Candles: Candle[],
  cfg: IndicatorConfig = {},
