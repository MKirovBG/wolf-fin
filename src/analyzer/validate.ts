...
interface ValidateInput {
  ...
  keyLevels:  KeyLevel[]
  atr:        number          // current ATR — used as proximity tolerance
  bias:       'bullish' | 'bearish' | 'neutral'
  mtfScore?:  number          // -3 to +3 confluence score (optional)
}

// ── Helpers ───────────────────────────────────────────────────────────────────

function distanceTo(price: number, level: KeyLevel): number {
  return Math.abs(price - level.price)
}

function nearestLevel(price: number, levels: KeyLevel[]): KeyLevel | null {
  if (levels.length === 0) return null
  return levels.reduce((best, l) =>
    distanceTo(price, l) < distanceTo(price, best) ? l : best
  )
}

// ── Validator ─────────────────────────────────────────────────────────────────

export function validateProposal(input: ValidateInput): ProposalValidation {
  ...
  const slLevels = keyLevels.filter(l =>
    isBuy  ? l.price < entryMid : l.price > entryMid
  )
  ...
  if (proposal.takeProfits.length > 0 && atr > 0) {
    ...
    const tpLevels = keyLevels.filter(l =>
      isBuy ? l.price > entryMid : l.price < entryMid
    )
