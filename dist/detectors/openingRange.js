...
export function detectOpeningRange(input) {
    ...
    if (n < ORB_BARS + 2)
        return emptyCandidate(features.symbolKey, KEY, LABEL, ['Insufficient candles for ORB']);
    // ── Hard disqualifiers ─────────────────────────────────────────────────────
    const disq = [];
    ...
    if (orbRange < atr * 0.3 || orbRange > atr * 2.5) {
        return emptyCandidate(features.symbolKey, KEY, LABEL, [`ORB range ${(orbRange / atr).toFixed(2)} ATR — outside 0.3–2.5 ATR window`]);
    }
    ...
    if (!bullBreak && !bearBreak) {
        return emptyCandidate(features.symbolKey, KEY, LABEL, ['Price has not broken out of the opening range yet']);
    }
...
//# sourceMappingURL=openingRange.js.map