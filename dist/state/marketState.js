...
function classifyRegime(features) {
    ...
    if (chochSignal) {
        ...
        if (volatility.recentRangeExpansion)
            reasons.push('Range expanding on CHoCH candle');
...
function classifyDirection(features) {
    ...
    if (trend.directionBias === 'bullish') {
        ...
        if (trend.macdBias === 'bullish')
            reasons.push('MACD histogram positive');
        if (trend.psarBias === 'bullish')
            reasons.push('PSAR below price (bullish)');
        if (trend.rsiZone === 'neutral' && trend.rsiValue > 52)
            reasons.push(`RSI ${trend.rsiValue.toFixed(0)} — bullish momentum zone`);
        if (trend.mtfAlignment === 'aligned_bullish')
            reasons.push('MTF score aligned bullish');
    }
    else if (trend.directionBias === 'bearish') {
        reasons.push(`EMA alignment: ${trend.emaAlignment}`);
        if (trend.macdBias === 'bearish')
            reasons.push('MACD histogram negative');
        if (trend.psarBias === 'bearish')
            reasons.push('PSAR above price (bearish)');
        if (trend.rsiZone === 'neutral' && trend.rsiValue < 48)
            reasons.push(`RSI ${trend.rsiValue.toFixed(0)} — bearish momentum zone`);
        if (trend.mtfAlignment === 'aligned_bearish')
            reasons.push('MTF score aligned bearish');
    }
    else {
...
function classifyVolatility(features) {
    const { volatility } = features;
    const reasons = [];
    reasons.push(`ATR at ${volatility.volatilityPercentile}th percentile`);
    if (volatility.recentRangeExpansion)
        reasons.push('Recent range expanding vs prior bars');
    if (volatility.keltnerPosition === 'above')
        reasons.push('Price above Keltner upper band');
    if (volatility.keltnerPosition === 'below')
...
function classifySession(features) {
    ...
    const reasons = [session.sessionNote];
    if (session.isLondonNYOverlap)
        reasons.push('London-NY overlap — highest liquidity window');
    else if (session.isOptimalSession)
        reasons.push('Optimal trading session active');
    else if (session.activeSessions.length === 0)
        reasons.push('Dead market — outside major sessions');
    return { quality: session.sessionQuality, reasons };
}
// ── Context risk ──────────────────────────────────────────────────────────────
function classifyContextRisk(features) {
    ...
    const risk = riskScore >= 4 ? 'avoid'
        : riskScore >= 3 ? 'elevated'
            : riskScore >= 1 ? 'moderate'
                : 'low';
    if (riskScore === 0)
        reasons.push('No elevated context risks');
    return { risk, reasons };
}
// ── Main export ───────────────────────────────────────────────────────────────
export function classifyMarketState(features) {
