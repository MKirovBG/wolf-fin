...
import type { BacktestTrade, BacktestMetrics } from '../types/research.js'

export function computeMetrics(trades: BacktestTrade[], totalBars: number): BacktestMetrics {
  ...
  for (const t of resolved) {
    if (t.outcome === 'lost_sl') { curConsec++; maxConsecLosses = Math.max(maxConsecLosses, curConsec) }
    else { curConsec = 0 }
  }

  // Group by setup type
  const bySetupType: BacktestMetrics['bySetupType'] = {}
  for (const t of resolved) {
    const k = t.setupType
    if (!bySetupType[k]) bySetupType[k] = { trades: 0, wins: 0, winRate: 0 }
    bySetupType[k].trades++
    if (t.outcome === 'won_tp1' || t.outcome === 'won_tp2') bySetupType[k].wins++
  }
  ...
  const bySession: BacktestMetrics['bySession'] = {}
  for (const t of resolved) {
    const sess = t.tags.find(tag => ['London', 'NY', 'London-NY', 'Tokyo', 'off-session'].some(s => tag.includes(s))) ?? 'unknown'
    if (!bySession[sess]) bySession[sess] = { trades: 0, wins: 0, winRate: 0 }
    bySession[sess].trades++
    if (t.outcome === 'won_tp1' || t.outcome === 'won_tp2') bySession[sess].wins++
  }
