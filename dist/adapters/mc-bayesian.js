...
function recencyWeight(tradeIndex, totalTrades, halfLifeTrades = 20) {
    const age = totalTrades - 1 - tradeIndex; // 0 = most recent
    return Math.exp(-Math.log(2) * age / halfLifeTrades);
}
// ── Layer 4: Bayesian Confidence ──────────────────────────────────────────────
export function runBayesian(trades) {
    if (trades.length === 0) {
        return {
            alpha: 2, beta: 2,
            posteriorMean: 0.5,
...
export function runSignificance(trades) {
...
export function runKelly(input) {
    ...
    const recommendedFraction = (() => {
        if (significance.edgeLabel === 'INSUFFICIENT_DATA')
            return 'No Trade';
        if (significance.edgeLabel === 'UNCONFIRMED')
            return 'No Trade';
        if (bayesian.confidence === 'LOW')
            return '1/4 Kelly';
        if (bayesian.confidence === 'MEDIUM')
            return '1/4 Kelly';
    ...
    const recommendedKellyPct = (() => {
        if (recommendedFraction === '1/4 Kelly')
            return quarterKellyPct;
        if (recommendedFraction === '1/2 Kelly')
            return halfKellyPct;
        return 0; // 'No Trade'
    })();
    // Compare against configured risk
    const riskAssessment = (() => {
        if (recommendedFraction === 'No Trade')
            return 'NO_EDGE';
        if (configuredRiskPct === null)
            return 'OPTIMAL';
        if (configuredRiskPct < recommendedKellyPct * 0.6)
            return 'UNDER_BETTING';
        if (configuredRiskPct > recommendedKellyPct * 1.5)
            return 'OVER_BETTING';
        return 'OPTIMAL';
    })();
...
//# sourceMappingURL=mc-bayesian.js.map