...
import type { MarketSnapshot, RiskState } from '../adapters/types.js'
import type { IMarketAdapter } from '../adapters/interface.js'

const __dirname = dirname(fileURLToPath(import.meta.url))
const RECORDING_DIR = join(__dirname, '../../data/recordings')

export function createRecordingAdapter(
  inner: IMarketAdapter,
  symbol: string,
): IMarketAdapter {
  ...
  return new Proxy(inner, {
    get(target, prop, receiver) {
      if (prop === 'getSnapshot') {
        return async (sym: string, risk: RiskState): Promise<MarketSnapshot> => {
