...
import type { StrategyFn, Signal } from './strategies.js'
...
export function runBacktest(recordingPath: string, strategy: StrategyFn): BacktestResult {
  const raw = readFileSync(recordingPath, 'utf-8').trim()
  const snapshots: MarketSnapshot[] = raw.split('\n').map(line => JSON.parse(line))
