...
// ── Component scorers ─────────────────────────────────────────────────────────
function scoreTrendAlignment(candidate, features, marketState) {
    const reasons = [];
    let s = 0;
    ...
    if ((dir === 'BUY' && trend.emaAlignment === 'bullish') ||
        ...
        s += 6;
        reasons.push('EMA aligned with trade direction');
    }
    else if (trend.emaAlignment === 'neutral') {
        s += 2;
        reasons.push('EMA neutral — partial credit');
    }
    // MTF alignment
    if ((dir === 'BUY' && trend.mtfAlignment === 'aligned_bullish') ||
        (dir === 'SELL' && trend.mtfAlignment === 'aligned_bearish')) {
        s += 5;
        reasons.push('MTF score fully aligned');
    }
    else if (trend.mtfAlignment === 'divergent') {
        s += 1;
        reasons.push('MTF partially aligned');
    }
    // ADX strength
    if (trend.adxStrength === 'strong') {
        s += 4;
        reasons.push('ADX strong — trend conviction');
    }
    else if (trend.adxStrength === 'moderate') {
        s += 2;
        reasons.push('ADX moderate');
    }
    return { score: Math.min(s, WEIGHTS.trendAlignment), reasons };
}
function scoreStructureQuality(candidate, features) {
    const reasons = [];
    let s = 0;
    const dir = candidate.direction;
    ...
    if ((dir === 'BUY' && structure.bos === 'bullish') ||
        ...
        s += 5;
        reasons.push('BOS in trade direction');
    }
    ...
    if ((dir === 'BUY' && structure.choch === 'bullish') ||
        ...
        s += 4;
        reasons.push('CHoCH structural confirmation');
    }
    return { score: Math.min(s, WEIGHTS.structureQuality), reasons };
}
function scoreEntryPrecision(candidate, features) {
    const reasons = [];
    let s = 0;
    if (!candidate.entryZone)
        return { score: 0, reasons: ['No entry zone defined'] };
    ...
    if (atr > 0) {
        ...
        if (widthRatio < 0.3) {
            s += 5;
            reasons.push('Tight entry zone (<0.3 ATR wide)');
        }
        else if (widthRatio < 0.6) {
            s += 3;
            reasons.push('Moderate entry zone (0.3–0.6 ATR)');
        }
        else {
            s += 1;
            reasons.push('Wide entry zone — less precise');
        }
    }
    // Near Fibonacci level
    if (levels.nearestFibLabel) {
        s += 3;
        reasons.push(`Entry near Fibonacci ${levels.nearestFibLabel}`);
    }
    // Near VWAP
    if (levels.vwapDistance < 0.1) {
        s += 2;
        reasons.push('Entry near VWAP — fair value zone');
    }
    return { score: Math.min(s, WEIGHTS.entryPrecision), reasons };
}
function scoreStopQuality(candidate, features) {
    const reasons = [];
    let s = 0;
    if (candidate.stopLoss === null || !candidate.entryZone)
        return { score: 0, reasons: ['No stop defined'] };
    const atr = features.volatility.atrAbsolute;
    ...
    if (atr > 0) {
        ...
        if (atrRatio >= 0.8 && atrRatio <= 1.5) {
            s += 7;
            reasons.push(`Stop at ${atrRatio.toFixed(2)} ATR — well-placed`);
        }
        else if (atrRatio >= 0.5 && atrRatio < 0.8) {
            s += 4;
            reasons.push(`Stop at ${atrRatio.toFixed(2)} ATR — tight but valid`);
        }
        else if (atrRatio > 1.5 && atrRatio <= 2.5) {
            s += 3;
            reasons.push(`Stop at ${atrRatio.toFixed(2)} ATR — wide`);
        }
        else {
            s += 1;
            reasons.push(`Stop at ${atrRatio.toFixed(2)} ATR — outside normal range`);
        }
    }
    // Behind structure
    const { structure } = features;
    ...
    if (stopBehindSwing) {
        s += 3;
        reasons.push('Stop behind swing structure');
    }
    return { score: Math.min(s, WEIGHTS.stopQuality), reasons };
}
function scoreTargetQuality(candidate, features) {
    const reasons = [];
    let s = 0;
    ...
    if (rr >= 3) {
        s += 7;
        reasons.push(`R:R ${rr.toFixed(1)} — excellent`);
    }
    else if (rr >= 2) {
        s += 5;
        reasons.push(`R:R ${rr.toFixed(1)} — good`);
    }
    else if (rr >= 1.5) {
        s += 3;
        reasons.push(`R:R ${rr.toFixed(1)} — acceptable`);
    }
    else {
        s += 1;
        reasons.push(`R:R ${rr.toFixed(1)} — low`);
    }
    if (candidate.targets.length >= 2) {
        s += 2;
        reasons.push('Multiple targets defined');
    }
    if (candidate.invalidationRule) {
        s += 1;
        reasons.push('Invalidation rule defined');
    }
    return { score: Math.min(s, WEIGHTS.targetQuality), reasons };
}
function scoreSessionTiming(candidate, features) {
    const reasons = [];
    let s = 0;
    const { session } = features;
    if (session.sessionQuality === 'optimal') {
        s = 10;
        reasons.push('London-NY overlap — peak liquidity');
    }
    else if (session.sessionQuality === 'favorable') {
        s = 7;
        reasons.push(`${session.activeSessions.join('+')} session`);
    }
    else if (session.sessionQuality === 'acceptable') {
        s = 4;
        reasons.push('Active session but not peak');
    }
    else {
        s = 1;
        reasons.push('Off-session — low liquidity');
    }
    return { score: s, reasons };
}
function scoreVolatilitySuitability(candidate, features) {
    const reasons = [];
    let s = 0;
    const { volatility } = features;
    if (volatility.volatilityRegime === 'normal') {
        s = 9;
        reasons.push('Normal volatility — optimal for execution');
    }
    else if (volatility.volatilityRegime === 'elevated') {
        s = 6;
        reasons.push('Elevated volatility — wider stops may be needed');
    }
    else if (volatility.volatilityRegime === 'quiet') {
        s = 4;
        reasons.push('Quiet volatility — possible expansion ahead');
    }
    else {
        s = 1;
        reasons.push('Abnormal volatility — reduced confidence');
    }
    if (volatility.keltnerPosition === 'inside') {
        s = Math.min(s + 1, WEIGHTS.volatilitySuitability);
    }
    return { score: s, reasons };
}
function scoreExecutionQuality(candidate, features) {
    const reasons = [];
    let s = 0;
    const { execution } = features;
    if (execution.spreadStatus === 'normal') {
        s = 10;
        reasons.push(`Spread ${execution.spreadPips.toFixed(1)} pips — normal`);
    }
    else if (execution.spreadStatus === 'wide') {
        s = 5;
        reasons.push(`Spread ${execution.spreadPips.toFixed(1)} pips — wide`);
    }
    else {
        s = 0;
        reasons.push(`Spread ${execution.spreadPips.toFixed(1)} pips — abnormal`);
    }
    return { score: s, reasons };
}
function scoreStrategyFit(candidate, features, marketState, strategy) {
    if (!strategy)
        return { score: 7, reasons: ['No strategy filter — default fit score'] };
    const reasons = [];
    let s = 0;
    ...
    const sessionOk = strategy.context.allowedSessions.length === 0 ||
        features.session.activeSessions.some(sess => strategy.context.allowedSessions.some(allowed => sess.toLowerCase().includes(allowed.toLowerCase())));
    ...
    if (detectorOk) {
        s += 4;
        reasons.push('Detector matches strategy');
    }
    if (regimeOk) {
        s += 3;
        reasons.push(`Regime '${marketState.regime}' allowed by strategy`);
    }
    if (sessionOk) {
        s += 2;
        reasons.push('Session matches strategy');
    }
    if (spreadOk) {
        s += 1;
        reasons.push('Spread within strategy limit');
    }
    if (!rrOk && candidate.riskReward > 0)
        reasons.push(`R:R ${candidate.riskReward.toFixed(2)} below strategy minimum ${strategy.riskRules.minRR}`);
    return { score: Math.min(s, WEIGHTS.strategyFit), reasons };
}
// ── Penalties ─────────────────────────────────────────────────────────────────
function computeContextPenalty(features) {
    const reasons = [];
    let p = 0;
    if (features.context.calendarRisk === 'medium') {
        p += 8;
        reasons.push('Medium calendar risk — penalty applied');
    }
    if (features.context.calendarRisk === 'low') {
        p += 3;
        reasons.push('Low calendar risk — minor penalty');
    }
    if (features.context.newsRisk === 'elevated') {
        p += 5;
        reasons.push('Elevated news sentiment — penalty');
    }
    return { penalty: -Math.min(p, WEIGHTS.contextPenalty), reasons };
}
function computeOverextensionPenalty(candidate, features) {
    const reasons = [];
    ...
    if (ext > 3) {
        p = 10;
        reasons.push(`Overextension ${ext.toFixed(1)} ATR — high reversal risk`);
    }
    else if (ext > 2) {
        p = 5;
        reasons.push(`Overextension ${ext.toFixed(1)} ATR — moderate risk`);
    }
    else if (ext > 1.5) {
        p = 2;
        reasons.push(`Overextension ${ext.toFixed(1)} ATR — minor risk`);
    }
    return { penalty: -Math.min(p, WEIGHTS.overextensionPenalty), reasons };
}
function computeCounterTrendPenalty(candidate, features, marketState) {
    if (!candidate.direction)
        return { penalty: 0, reasons: [] };
    const reasons = [];
    const dir = candidate.direction;
    const { trend, structure } = features;
    const isCounterTrend = (dir === 'BUY' && trend.directionBias === 'bearish' && marketState.regime === 'trend') ||
    ...
    if (structure.choch === null) {
        p += 3;
        reasons.push('No CHoCH — counter-trend without reversal signal');
    }
    return { penalty: -Math.min(p, WEIGHTS.counterTrendPenalty), reasons };
}
// ── Main scorer ───────────────────────────────────────────────────────────────
export function scoreCandidate(candidate, features, marketState, strategy) {
    ...
    allReasons.push(...trendRes.reasons, ...structRes.reasons, ...entryRes.reasons, ...stopRes.reasons, ...targetRes.reasons, ...sessionRes.reasons, ...volRes.reasons, ...execRes.reasons, ...stratRes.reasons, ...ctxPenalty.reasons, ...extPenalty.reasons, ...ctrPenalty.reasons);
    const breakdown = {
 ...
 */
export function scoreCandidates(candidates, features, marketState, strategy) {
    return candidates
